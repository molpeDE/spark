...
/**
95% Based on https://github.com/tkrajina/typescriptify-golang-structs
...
func New() *TypeScriptify {
	...
	kinds[reflect.Int] = "number /* int */"
	kinds[reflect.Int8] = "number /* int8 */"
	kinds[reflect.Int16] = "number /* int16 */"
	kinds[reflect.Int32] = "number /* int32 */"
	kinds[reflect.Int64] = "number /* int64 */"
	kinds[reflect.Uint] = "number /* uint */"
	kinds[reflect.Uint8] = "number /* uint8 */"
	kinds[reflect.Uint16] = "number /* uint16 */"
	kinds[reflect.Uint32] = "number /* uint32 */"
	kinds[reflect.Uint64] = "number /* uint64 */"
	kinds[reflect.Float32] = "number /* float32 */"
	kinds[reflect.Float64] = "number /* float64 */"
...
func (t *TypeScriptify) getJSONFieldName(field reflect.StructField, isPtr bool) string {
	...
	if len(jsonTag) > 0 {
		...
		jsonFieldName = fmt.Sprintf("\"%s\"", field.Name)
	}
...
func (t *TypeScriptify) convertType(depth int, typeOf reflect.Type, customCode map[string]string) (string, error) {
	...
	for _, field := range fields {
		...
		if fldOpts.TSTransform != "" {
			...
			if field.Type.Name() == "" { // Hack for inline structs
				oldSuffix := t.Suffix
				oldBuilderSuffix := builder.suffix
				t.Suffix = fmt.Sprintf("__INLINE_%s", randomHex(8))
				builder.suffix = t.Suffix
				t.logf(depth, "--- USING INLINE HACK ---> %s", t.Suffix)
				...
				t.Suffix = oldSuffix
				builder.suffix = oldBuilderSuffix
